...
\begin{itemize}
    ...
    \end{itemize}
    引入基于 TarDAL 的双判别器机制是为了在“检测驱动”的强监督下，保持图像的原始辐射强度分布与纹理完整性。与仅依赖语义梯度回传的方案不同，本项目引入的对抗损失（$L_{adv}$）作为一种**分布正则项**，其核心逻辑在于约束融合网络在优化检测精度的同时，不偏离红外热辐射和可见光纹理的原始流形，从而有效抑制了“任务过拟合”带来的非自然伪影。
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/placeholder_visual_compare.png} % PLACEHOLDER
    \caption{与主流 SOTA 方法 (DenseFuse, TarDAL, SeAFusion) 的视觉效果对比。红色框标注了行人目标的细节恢复情况，绿色框标注了背景标识牌的清晰度。本文方法在保留红外高亮特性的同时，有效抑制了可见光过曝带来的光晕。}
...
\begin{table}[H]
    ...
    \begin{tabular}{l|cccc}
        \toprule
        \textbf{Method} & \textbf{SF} $\uparrow$ & \textbf{AG} $\uparrow$ & \textbf{VIF} $\uparrow$ & \textbf{SSIM} $\uparrow$ \\
        \midrule
        DenseFuse \cite{li2018densefuse} & 12.45 & 3.21 & 0.65 & 0.72 \\
        FusionGAN \cite{ma2019fusiongan} & 10.33 & 2.89 & 0.58 & 0.68 \\
        SeAFusion \cite{tang2022seafusion} & 14.12 & 4.05 & 0.78 & 0.76 \\
        TarDAL (Baseline) & 13.89 & 3.98 & 0.75 & 0.79 \\
        \textbf{Ours} & \textbf{---} & \textbf{---} & \textbf{---} & \textbf{---} \\
        \bottomrule
    \end{tabular}
    \small \\ \textit{*注：由于Ours模型的完整训练尚未结束，此处使用占位符，待最终测试后填入。}
\end{table}
...
\textbf{结果分析}：
\begin{itemize}
    \item **SE vs CA**: SE Block 仅关注通道权重，忽略了空间位置，导致 mAP 提升有限 (+0.3\%)。
    \item **CBAM vs CA**: CBAM 虽然引入了空间注意力，但它是通过 $7 \times 7$ 卷积提取的局部特征，对于跨越整幅图像的长距离依赖（如延伸的车道线）捕捉能力不如 CA 的 X/Y 方向池化。
    \item **CA 的优势**: Coordinate Attention 显式地对水平和垂直方向进行编码，通过精确捕捉道路和行人的正交结构特征，实现了最优的检测精度 (+1.8\%)。
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/placeholder_feature_map.png} % PLACEHOLDER
    \caption{特征响应热力图对比：(a) Baseline 模型特征关注点较为散乱，对背景强光源敏感；(b) Ours (w/ CA) 模型精准聚焦于行人的躯干（垂直响应）和路沿（水平响应），背景噪声显著抑制。}
    \label{fig:heatmap}
\end{figure}
